...
public class GestorEntidadesJPA {
    ...
    public static void modificarSeccion() {
        String idSeccion = EntradaTeclado.cadenaLimitada("Código de sección:", 0, 2);
    ...
    public static void insertarEmpleado() {
        ...
        if (seccion == null) {
            System.out.println(Color.rojo("ERROR: La sección no existe."));
            return;
        }
    ...
    /**
     * Lista todos los empleados, opcionalmente filtrados por sección.
     */
    public static void listarEmpleados() {
        List<Empleado> empleados = empleadoDAO.listarTodos();
        String idSeccion = EntradaTeclado.cadenaLimitada("Código de sección (dejar en blanco para todas):", 0, 2);

        System.out.println(Color.azul(String.format("%-6s %-30s %12s %15s", "CÓDIGO", "NOMBRE", "SALARIO", "SECCIÓN")));

        for (Empleado e : empleados) {
            if (idSeccion.equals(e.getSeccion().getIdSeccion()) || idSeccion.isEmpty()) {
                System.out.printf("%-6s %-30s %12d %15s%n",
                        e.getIdEmpleado(),
                        e.getNombre(),
                        e.getSalarioAnual(),
                        e.getSeccion().getDescripcion());
            }
        }
    }
    ...
    public static void modificarEmpleado() {
        ...
        String nombre = EntradaTeclado.cadenaLimitada("Nuevo nombre, [ENTER] para dejar igual:", 0, 30);
        if (!nombre.isEmpty()) {
            empleado.setNombre(nombre);
        }
        ...
        if (!idSeccion.isEmpty()) {
            ...
            if (seccion != null) {
                ...
                System.out.println(Color.rojo("ERROR: La nueva sección no existe."));
            }
        }
